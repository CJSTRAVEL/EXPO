...
const ClientPortal = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5]">
      ...
      {/* Tabs */}
      <div className="bg-[#1a1a1a]/90 border-b border-[#D4A853]/20 overflow-x-auto">
        <div className="max-w-7xl mx-auto px-2 sm:px-4">
          <div className="flex gap-0 sm:gap-1 min-w-max">
            {[
              { id: "bookings", label: "Bookings", fullLabel: "Confirmed Bookings", icon: Car },
              { id: "requests", label: "Pending", fullLabel: "Pending Requests", icon: Clock3 },
              { id: "invoices", label: "Invoices", fullLabel: "Invoices", icon: Receipt },
              { id: "history", label: "History", fullLabel: "History", icon: History },
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex items-center gap-1 sm:gap-2 px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium transition-colors border-b-2 whitespace-nowrap ${
                  activeTab === tab.id
              ...
              >
                ...
                {tab.id === "invoices" && invoiceStats.unpaid > 0 && (
                  <span className="ml-0.5 sm:ml-1 px-1 sm:px-1.5 py-0.5 text-[10px] sm:text-xs bg-red-500 text-white rounded-full">
                    {invoiceStats.unpaid}
                  </span>
                )}
              </button>
